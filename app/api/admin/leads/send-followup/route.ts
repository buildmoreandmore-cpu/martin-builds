...
export async function POST(req: NextRequest) {
  try {
    const { lead_id, lead_email, lead_name, type } = await req.json();

    if (!lead_email || !lead_name) {
      return NextResponse.json({ error: "Missing lead info" }, { status: 400 });
    }

    const emailType = type === "proposal" ? "proposal" : "initial";
    const firstName = lead_name.split(" ")[0] || "there";
    const html = buildFollowUpEmail(firstName, emailType as "initial" | "proposal");
