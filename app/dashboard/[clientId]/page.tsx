...
export default function ClientDashboard() {
  ...
  // Fetch client info
  useEffect(() => {
    fetch(`/api/dashboard/messages?clientId=${clientId}&limit=1`)
      .then((r) => r.json())
      .then(() => {
        // We'll get client info from a separate call or just use what we have
      })
      .catch(() => {});
  }, [clientId]);
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", fontFamily: fontBody, color: textLight, background: bg }}>
      <style>{`
        ...
        .dash-input:focus { outline: none; border-color: ${accent} !important; }
      `}</style>
        ...
        <div style={{ padding: "0 20px", marginBottom: 32 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 30, height: 30, borderRadius: 8, background: accent, display: "flex", alignItems: "center", justifyContent: "center" }}>
              <span style={{ fontSize: 13, fontWeight: 800, color: "#111", fontFamily: fontMono }}>P</span>
            </div>
            <div>
              <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: "#fff", lineHeight: 1.2 }}>Parker</p>
              <p style={{ margin: 0, fontSize: 11, color: muted }}>PCG Screening</p>
            </div>
          </div>
        </div>
        ...
        <nav style={{ flex: 1, display: "flex", flexDirection: "column", gap: 2, padding: "0 8px" }}>
          {NAV.map((item) => {
            ...
            return (
              ...
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={active ? "#fff" : "#666"} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                  <path d={item.icon} />
                </svg>
      ...
      {/* Main */}
      <main className="dash-main" style={{ flex: 1, marginLeft: 200, display: "flex", flexDirection: "column", height: "100vh" }}>
        ...
        {view === "chat" && (
          <>
            ...
            <div style={{ padding: "16px 24px", borderBottom: `1px solid ${border}`, display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 600 }}>Parker</span>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "24px 24px 12px" }}>
              {messages.length === 0 && (
                <div style={{ textAlign: "center", padding: "60px 20px", color: muted }}>
                  <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#333" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ margin: "0 auto 16px", display: "block" }}>
                    <path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z" />
                  </svg>
                  <p style={{ fontSize: 14, margin: "0 0 4px", fontWeight: 600, color: "#555" }}>No messages yet</p>
                  <p style={{ fontSize: 13, margin: 0 }}>Send a message to start talking to Parker</p>
                </div>
              )}
                  ...
                  <div key={msg.id}>
                    {showDate && (
                      <div style={{ textAlign: "center", margin: "20px 0 12px" }}>
                        <span style={{ fontSize: 11, color: muted, fontFamily: fontMono, background: "#111116", padding: "3px 10px", borderRadius: 6 }}>
              ...
              {sending && (
                <div style={{ display: "flex", justifyContent: "flex-start", marginBottom: 10 }}>
                  <div style={{ padding: "12px 16px", borderRadius: "16px 16px 16px 4px", background: card, fontSize: 14, color: muted }}>
                    Thinking...
                  </div>
                </div>
              )}
            ...
            <div style={{ padding: "12px 24px 20px", borderTop: `1px solid ${border}` }}>
              ...
              >
                ...
                <button
                  ...
                  style={{
                    padding: "12px 20px",
                    background: input.trim() ? accent : "#333",
        ...
        {view === "activity" && (
          <div style={{ padding: 40, textAlign: "center" }}>
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#333" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ margin: "60px auto 16px", display: "block" }}>
              <path d="M22 12h-4l-3 9L9 3l-3 9H2" />
            </svg>
            <p style={{ fontSize: 16, fontWeight: 600, color: "#555" }}>Activity</p>
            <p style={{ fontSize: 13, color: muted }}>Conversation analytics and lead tracking coming soon.</p>
          </div>
        )}

        {view === "settings" && (
          <div style={{ padding: 40 }}>
            <h2 style={{ fontSize: 18, fontWeight: 700, margin: "60px 0 24px" }}>Settings</h2>
            <div style={{ display: "flex", flexDirection: "column", gap: 16, maxWidth: 400 }}>
              {[
                { label: "Business", value: "PCG Screening Services" },
                { label: "Agent", value: "Parker" },
